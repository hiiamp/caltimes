<li data-name="{{$task->name}}" data-index="{{$task->id}}" data-position="{{$task->position}}"
    data-status="{{$task->todo_list_id}} " class="has-dropdown">
    <p>
    ...
    <p id="priority1{{$task->id}}" class="badge js-due-date-badge is-due-past"
       ...
       @else title="High Priority"
        ...
        <span id="character{{$task->id}}" class="badge-text2 js-due-date-text" title="{{$task->assign->name}}"
